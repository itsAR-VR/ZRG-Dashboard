...
## Evidence
- `npm test` — pass (`401` tests, `0` failures).
...
- Booking-route signaling now uses AI router JSON as the source of truth (Process 4/5 map directly to Slack action signals); deterministic keyword detections are fallback-only when routing is unavailable.
