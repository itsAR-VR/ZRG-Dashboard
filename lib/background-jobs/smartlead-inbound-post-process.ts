...
export async function runSmartLeadInboundPostProcessJob(params: {
  ...
  if (!message) {
    console.error("[SmartLead Post-Process] Message not found:", params.messageId);
    return;
  }

  if (!message.lead) {
    console.error("[SmartLead Post-Process] Lead not found for message:", params.messageId);
    return;
  }
  ...
  if (message.direction === "outbound") {
    console.log("[SmartLead Post-Process] Skipping outbound message");
    return;
  }
  ...
  console.log("[SmartLead Post-Process] Sentiment:", sentimentTag, "Status:", leadStatus);
  ...
  if (snoozeKeywordHit) {
    ...
    if (snoozedUntilUtc && confidence >= 0.95) {
      ...
      console.log("[SmartLead Post-Process] Snoozed until", snoozedUntilUtc.toISOString());
    }
  }
  ...
  if (autoBook.booked) {
    console.log("[SmartLead Post-Process] Auto-booked appointment:", autoBook.appointmentId);
  }
  ...
  if (!autoBook.booked && shouldGenerateDraft(sentimentTag, lead.email)) {
    console.log("[SmartLead Post-Process] Generating draft for message", message.id);
    ...
    if (draftResult.success) {
      ...
      if (responseMode === "AI_AUTO_SEND" && draftId && draftContent) {
        ...
        if (evaluation.safeToSend && evaluation.confidence >= autoSendThreshold) {
          console.log("[SmartLead Post-Process] Auto-send approved for draft", draftId, "confidence", evaluation.confidence.toFixed(2));
          ...
          if (delayConfig && (delayConfig.delayMinSeconds > 0 || delayConfig.delayMaxSeconds > 0)) {
            ...
            if (scheduleResult.scheduled) {
              console.log("[SmartLead Post-Process] Scheduled delayed send for draft", draftId, "runAt:", scheduleResult.runAt?.toISOString());
            } else {
              console.log("[SmartLead Post-Process] Delayed send not scheduled:", scheduleResult.skipReason);
            }
          } else {
            ...
            if (!validation.proceed) {
              console.log(
                "[SmartLead Post-Process] Skipping immediate auto-send for draft",
                draftId,
              ...
              if (sendResult.success) {
                console.log("[SmartLead Post-Process] Sent message:", sendResult.messageId);
              } else {
          ...
          console.log("[SmartLead Post-Process] Auto-send blocked:", evaluation.reason);
        }
        ...
        if (!decision.shouldReply) {
          console.log("[SmartLead Post-Process] Skipped auto-send for lead", lead.id, decision.reason);
        } else {
          console.log("[SmartLead Post-Process] Auto-approving draft", draftId, "for lead", lead.id);
          ...
          if (sendResult.success) {
            ...
            console.error("[SmartLead Post-Process] Failed to send draft:", sendResult.error);
          }
        }
      } else {
    ...
    console.log("[SmartLead Post-Process] Skipping draft generation (sentiment:", sentimentTag, "auto-booked:", autoBook.booked, ")");
  }
  ...
  try {
    ...
    console.error("[SmartLead Post-Process] Failed to enqueue lead scoring job for lead", lead.id, error);
  }
