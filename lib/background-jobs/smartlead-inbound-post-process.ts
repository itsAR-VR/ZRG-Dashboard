    ...
    if (draftResult.success) {
      ...
      if (!draftId) {
        ...
        if (
          ...
          console.log(
            "[SmartLead Post-Process] Auto-send approved for draft",
            draftId,
            "confidence",
        ...
        if (
          ...
          switch (autoSendResult.outcome.action) {
            case "send_delayed": {
              console.log(
                "[SmartLead Post-Process] Scheduled delayed send for draft",
                draftId,
                "runAt:",
            ...
            case "send_immediate": {
              console.log("[SmartLead Post-Process] Sent message:", autoSendResult.outcome.messageId);
              break;
            }
            case "needs_review": {
            ...
            case "skip": {
              if (autoSendResult.telemetry.delayedScheduleSkipReason) {
                console.log("[SmartLead Post-Process] Delayed send not scheduled:", autoSendResult.telemetry.delayedScheduleSkipReason);
                ...
                console.log("[SmartLead Post-Process] Skipped auto-send for lead", lead.id, legacyReason);
              }
              break;
            }
            case "error": {
              if (autoSendResult.mode === "AI_AUTO_SEND") {
                console.error("[SmartLead Post-Process] Auto-send failed:", autoSendResult.outcome.error);
              } else {
                console.error("[SmartLead Post-Process] Failed to send draft:", autoSendResult.outcome.error);
              }
              break;
            }
          }
