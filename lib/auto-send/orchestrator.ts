...
export function createAutoSendExecutor(deps: AutoSendDependencies): { executeAutoSend: (context: AutoSendContext) => Promise<AutoSendResult> } {
  ...
  async function executeAiAutoSendPath(context: AutoSendContext, startTimeMs: number): Promise<AutoSendResult> {
    ...
    let evaluation = await deps.evaluateAutoSend({
      ...
      actionSignalCallRequested: context.actionSignalCallRequested ?? false,
      actionSignalExternalCalendar: context.actionSignalExternalCalendar ?? false,
      actionSignalRouteSummary: context.actionSignalRouteSummary ?? null,
  ...
  async function executeAutoSend(context: AutoSendContext): Promise<AutoSendResult> {
    ...
    if (context.actionSignalCallRequested === true || context.sentimentTag === "Call Requested") {
      const threshold =
        mode === "AI_AUTO_SEND"
      ...
      const reason =
        context.actionSignalCallRequested === true
          ? context.leadPhoneOnFile === true
            ? "call_requested_phone_on_file"
            : "call_requested_phone_missing"
          : "call_requested_sentiment";
