...
export const cronEmailBisonAvailabilitySlotFunction = inngest.createFunction(
  {
    id: "cron-emailbison-availability-slot",
    retries: 3,
    concurrency: { limit: 1 },
    idempotency: "event.data.dispatchKey",
  },
  { event: INNGEST_EVENT_CRON_EMAILBISON_AVAILABILITY_SLOT_REQUESTED },
  async ({ event, step, attempt, runId }) => {
    ...
    await writeInngestJobStatus({
      jobName: "cron-emailbison-availability-slot",
      status: "running",
      attempt,
    ...
    try {
      ...
      const result = await step.run("cron-emailbison-availability-slot", async () =>
        runEmailBisonAvailabilitySlotCron(searchParams)
      );
